...
logit offshoring `varlist_where_1'
eststo: margins, dydx(`varlist_where_1')  post	
su offshoring
estadd scalar avg = 100*r(mean)

logit offshoring `varlist_where_2'
eststo: margins, dydx(`varlist_where_2')  post	
su offshoring
estadd scalar avg = 100*r(mean)
estadd local FE_3 "\checkmark"

logit offshoring `varlist_where_3'
eststo: margins, dydx(`varlist_where_3')  post	
su offshoring
estadd scalar avg = 100*r(mean)
estadd local FE_3 "\checkmark"

logit offshoring `varlist_where_4'
eststo: margins, dydx(`varlist_where_4')  post	
su offshoring
estadd scalar avg = 100*r(mean)
estadd local FE_3 "\checkmark"

logit offshoring `varlist_where_5'
eststo: margins, dydx(`varlist_where_5')  post	
su offshoring
estadd scalar avg = 100*r(mean)
estadd local FE_3 "\checkmark"
...
** Paper Table
local note  "This table reports average marginal effects of the logit estimation of Equation \ref{eq:reg_where}. Covariates are relative to year 2017. We include one observation per firm $\times$ business function $\times$ destination."
local title "Where are business functions offshored?"
local label_table "reg_where_paper" 
...
** Slides Table
local label_table "reg_where_slides" 
...
order(log_avg_dist log_gdp_per_capita task_HS_fracxlog_gdp_per_capita task_HS_fracxINDE task_RD_fracxlog_gdp_per_capita task_RD_fracxINDE) ///
label booktabs compress nonotes nomtitle ///
prehead(`"\begin{table}[htbp] \footnotesize \centering"' `"\def\sym#1{\ifmmode^{#1}\else\(^{#1}\)\fi}"' `"\renewcommand{\arraystretch}{1}"' `"\begin{tabular}{l*{@M}{c}}"' `"\toprule"' `"&\multicolumn{@M}{c}{Dep. Var = 1 if $ b $ is offshored in $ d $} \\"' `"\cmidrule(lr){2-@span}"' ) ///
postfoot("\bottomrule \end{tabular}\end{table}") ///
stats(avg FE_1 FE_2 FE_3 N, fmt(%5.3g 0 0 0 %9.0fc) label("Average (\%)" "Firm controls" "Business function fixed effects" "Destination fixed effects" "Observations")) ///
star(* 0.10 ** 0.05 *** 0.01) substitute("\_ _")

************** Plot fixed effects
logit offshoring `varlist_firm' i.task_num destination_1 destination_2 destination_3 destination_4 destination_5 destination_6 destination_7 destination_8 destination_9
eststo offshoring: margins, dydx(`varlist_firm' i.task_num destination_1 destination_2 destination_3 destination_4 destination_5 destination_6 destination_7 destination_8 destination_9)  post

coefplot offshoring, keep( destination_1 destination_2 destination_3 destination_4 destination_5 destination_6 destination_7 destination_8 destination_9)  xtitle("Average marginal effect") xlabel(, labsize(medium)) ylabel(, labsize(medium)) xline(0) 
gr export "$output_path/figures/fixed_effects_where.pdf", as(pdf) replace
